...
function xmldb_bigbluebuttonbn_upgrade($oldversion = 0) {
    ...
    if ($oldversion < 2023122200) {
        ...
        $field = new xmldb_field('downloadpresentationwithannotations',
            XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, 0, 'chat');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        $field = new xmldb_field('snapshotofcurrentslide',
            XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, 0, 'downloadpresentationwithannotations');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        $field = new xmldb_field('externalvideos',
            XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, 0, 'snapshotofcurrentslide');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        $field = new xmldb_field('importpresentationwithannotationsfrombreakoutrooms',
            XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, 0, 'externalvideos');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        $field = new xmldb_field('importsharednotesfrombreakoutrooms',
            XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, 0, 'importpresentationwithannotationsfrombreakoutrooms');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        $field = new xmldb_field('layouts',
            XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, 0, 'importsharednotesfrombreakoutrooms');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        ...
        $field = new xmldb_field('sharednotes',
            XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, 0, 'screenshare');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        $field = new xmldb_field('virtualbackgrounds',
            XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, 0, 'sharednotes');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        $field = new xmldb_field('customvirtualbackgrounds',
            XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, 0, 'virtualbackgrounds');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        $field = new xmldb_field('presentationdf',
            XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, 0, 'customvirtualbackgrounds');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        ...
        // Bigbluebuttonbn savepoint reached.
        upgrade_mod_savepoint(true, 2023122200, 'bigbluebuttonbn');
    }
    return true;
}
